...
During one compatibility release, migration splits the first legacy `WorkspaceLayoutDocument` into a frame and workspace view. Later legacy workspaces contribute only resource placements mapped by visual group order; their geometry/tools cannot overwrite the already-local frame. Presence and absence are marked attempted. This module exposes pure conversion helpers but performs no legacy request itself. Current-layout revision/conflict semantics have no role in the new path.
