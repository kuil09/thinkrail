...
- **Public surface (`index.ts`):** frame/view types; workbench renderer/controller; pure mutations, projection, validation, and migration helpers; built-in preset definitions plus instantiate/apply/capture operations; attention fallback helpers; and unavailable-reason results. Callers inject resource renderers and commit complete pure results rather than splicing arrays.
- **External deps:** `@thinkrail/contracts` for the resource-free custom-preset DTO only; shell-neutral `lib` attention/id primitives; React; `react-resizable-panels`; `@dnd-kit/core`.
...
During one compatibility release, migration splits the first legacy `WorkspaceLayoutDocument` into a frame and workspace view. Later legacy workspaces contribute only resource placements mapped by visual group order; their geometry/tools cannot overwrite the already-local frame. Presence and absence are marked attempted. This module exposes pure conversion helpers but performs no legacy request itself. [[submodule-web-shell-layout-sync]] and current-layout revision/conflict semantics have no role in the new path.
