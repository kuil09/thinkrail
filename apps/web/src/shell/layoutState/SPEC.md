...
- **External deps:** browser storage/session primitives and React. `@thinkrail/contracts` is temporary and type-only for the legacy snapshot read plus the durable custom-preset DTO; current frame/view types are web-local.
...
The live state belongs to one browser tab or native window. Browser keys are qualified by transport endpoint and a session-restorable surface id; native adapters use `{ backendProfileId, windowId }`. The shell initializes this state even on Welcome, before any workspace is open, so local default/limit edits are immediately durable; a pristine bootstrap still yields to the first legacy snapshot during the compatibility import. Simultaneous surfaces never consume each other's storage events. Invalid or future local schemas fall back to the local default preset; persistence failure leaves live state usable.
